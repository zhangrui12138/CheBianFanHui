...
///@}
/*PRIZE-three-fingers-shotscreen-huangdianjun-2015-04-06-start*/
import android.content.Intent;
import android.provider.Settings;
/*PRIZE-three-fingers-shotscreen-huangdianjun-2015-04-06-end*/
//add for status bar inverse style. prize-linkh-20150903
...
public class PhoneWindow extends Window implements MenuBuilder.Callback {
    ...
    //private boolean mFirstOver;

    //private boolean isThreeFingersSlide;

    private static final String ACTION = "org.android.broadcastreceiverregister.SLIDESCREENSHOT";

    // rewrite codes for three-fingers-screenshot. prize-linkh-20160804
    private boolean mDbgSlidingForScreenShot = true;
    private boolean mAllowThreeFingersSlidingForScreenShot;
    private boolean mHasEatThreeFingers;
    private boolean mHasStartedScreenShot;
    private boolean mAreThreeFingerssSliding;
    private float mSlidingDistanceThreshold;
    //END...
    /*PRIZE-three-fingers-shotscreen-huangdianjun-2015-04-06-end*/
	...
	//add start:prize -process error press -lihuangyuan-20160526
	private long mHookHeadsetPressTime = 0;
	//add end:prize -process error press -lihuangyuan-20160526
        ...
        if (mPanelChordingKey != 0) {
            ...
            if (playSoundEffect) {
                ...
                if (audioManager != null) {
                    //audioManager.playSoundEffect(AudioManager.FX_KEY_CLICK); //perform this opetion in PhoneWindowManger. prize-linkh-20150519
                } else {
    ...
    protected boolean onKeyDown(int featureId, int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_MEDIA_FAST_FORWARD: {
                   //add start:prize -process error press -lihuangyuan-20160526
                  //Log.d(TAG,"keycode:"+event.getKeyCode()+",action:"+event.getAction());
                  if(KeyEvent.KEYCODE_HEADSETHOOK == event.getKeyCode())
                  {
                      mHookHeadsetPressTime = System.currentTimeMillis();
                  }
                  //add end:prize -process error press -lihuangyuan-20160526
                if (mMediaController != null) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_MEDIA_FAST_FORWARD: {
				...
				if(keyCode == KeyEvent.KEYCODE_HEADSETHOOK)
				{
					...
					curtime = curtime - mHookHeadsetPressTime;
					Log.e(TAG, "onKeyup "+"hookheadset down--up time:"+curtime);
					if(curtime < 100)
					{
						return true;
					}
    ...
    private final class DecorView extends FrameLayout implements RootViewSurfaceTaker {
        ...
        public boolean dispatchTouchEvent(MotionEvent ev) {
            ...
            /*
            int mSlideScreenshot = Settings.System.getInt(getContext().getContentResolver(),Settings.System.PRIZE_SLIDE_SCREENSHOT, 0);
            if(mSlideScreenshot == 1){
            
                switch (ev.getAction() & ev.ACTION_MASK) {
                	case MotionEvent.ACTION_DOWN:
                		isThreeFingersSlide = false;
                		break;
                    case MotionEvent.ACTION_POINTER_DOWN:
                        Log.v("hdj", ">>>>---------ACTION_POINTER_DOWN");
                        if (ev.getPointerCount() == 3) {
                        	 Log.v("hdj", ">>>>---------3 pointers ACTION_POINTER_DOWN");
                            mFirstPointStartY = ev.getY(0);
                            mSecondPointStartY = ev.getY(1);
                            mThirdPointStartY = ev.getY(2);
                            isThreeFingersSlide = true;
                            MotionEvent eventClone = MotionEvent.obtain(ev);
                            eventClone.setAction(MotionEvent.ACTION_CANCEL);
                            boolean flag = cb != null && !isDestroyed() && mFeatureId < 0 ? cb.dispatchTouchEvent(eventClone)
                             : super.dispatchTouchEvent(eventClone); 
                            return true;
                        }
                        break;
                    case MotionEvent.ACTION_MOVE:
                        if (ev.getPointerCount() == 3) {
                            ...
                            getWindowManager().getDefaultDisplay().getMetrics(
                                    mDisplayMetrics);
                            mDefaultDistance = (float) (mDisplayMetrics.heightPixels * 0.2);
                            if (!mFirstOver){
                                if (ev.getY(0) - mFirstPointStartY > mDefaultDistance
                                        && ev.getY(1) - mSecondPointStartY > mDefaultDistance
                                        && ev.getY(2) - mThirdPointStartY > mDefaultDistance) {
                                    mFirstOver = true;
                                    Log.v("huangdj", "***************");
                                    Context context = getContext();
                        ...
                        if(isThreeFingersSlide) {
                        	return true;
                        }
                        break;
                    case MotionEvent.ACTION_POINTER_UP:
                    	Log.v("hdj", ">>>>--ACTION_POINTER_UP, isThreeFingersSlide=" + isThreeFingersSlide);
                    	 if(isThreeFingersSlide){
                             MotionEvent eventClone = MotionEvent.obtain(ev);
                             eventClone.setAction(MotionEvent.ACTION_CANCEL);  
                             boolean flag = cb != null && !isDestroyed() && mFeatureId < 0 ? cb.dispatchTouchEvent(eventClone)
                    ...
                    case MotionEvent.ACTION_UP:
                    	Log.v("hdj", ">>>>--ACTION_CANCEL or ACTION_UP, isThreeFingersSlide=" + isThreeFingersSlide);
                    	if(isThreeFingersSlide){
                    		isThreeFingersSlide = false;
                    		mFirstOver = false;
                            MotionEvent eventClone = MotionEvent.obtain(ev);
            ...
            /*PRIZE-three-fingers-shotscreen-huangdianjun-2015-08-24-end*/
            
            // rewrite codes for three-fingers-screenshot. prize-linkh-20160804
            switch (ev.getAction() & ev.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN: {
                // Only check this status when receiving first down event.
                mAllowThreeFingersSlidingForScreenShot = Settings.System.getInt(
                    getContext().getContentResolver(),Settings.System.PRIZE_SLIDE_SCREENSHOT, 0) == 1;

                // reset everthing
                mAreThreeFingerssSliding = false;
                mHasEatThreeFingers = false;
                mHasStartedScreenShot= false;
                
                if (mDbgSlidingForScreenShot) {
                    Log.d(TAG, "ACTION_DOWN: allowScreenShot=" + mAllowThreeFingersSlidingForScreenShot);
                }
				//add by zhangrui start
				fanhuix1 = ev.getX();  
                fanhuiy1 = ev.getY();
            ...
            case MotionEvent.ACTION_POINTER_DOWN: {
                if (mAllowThreeFingersSlidingForScreenShot) {
                    if (mDbgSlidingForScreenShot) {
                        Log.d(TAG, "ACTION_POINTER_DOWN: fingers ? " + ev.getPointerCount());
                    }
                    if (ev.getPointerCount() == 3) {
                        if (mDbgSlidingForScreenShot) {
                            Log.d(TAG, "ACTION_POINTER_DOWN: detect 3 fingers.");
                        }
                        mFirstPointStartY = ev.getY(0);
                        mSecondPointStartY = ev.getY(1);
                        mThirdPointStartY = ev.getY(2);                        
                        mAreThreeFingerssSliding = true;
                        mHasEatThreeFingers = true;

                        DisplayMetrics mDisplayMetrics = new DisplayMetrics();
                        getWindowManager().getDefaultDisplay().getMetrics(
                                mDisplayMetrics);
                        mSlidingDistanceThreshold = (float)(mDisplayMetrics.heightPixels * 0.2);
                        if (mDbgSlidingForScreenShot) {
                            Log.d(TAG, "ACTION_POINTER_DOWN: mSlidingDistanceThreshold=" + mSlidingDistanceThreshold);
                        }
                        
                        // contruct cancel event to notify child views that this gesture is abandoned by system.
                        if (mDbgSlidingForScreenShot) {
                            Log.d(TAG, "ACTION_POINTER_DOWN: contruct cancel event to deliver to child views.");
                        }                        
                        MotionEvent eventClone = MotionEvent.obtain(ev);
                        eventClone.setAction(MotionEvent.ACTION_CANCEL);
                        boolean handled = cb != null && !isDestroyed() && mFeatureId < 0 ? cb.dispatchTouchEvent(eventClone)
                                        : super.dispatchTouchEvent(eventClone);
                        if (mDbgSlidingForScreenShot) {
                            Log.d(TAG, "ACTION_POINTER_DOWN: handled by child views ? " + handled);
                        }
                        
                        return true;
                    } else if (mAreThreeFingerssSliding || mHasEatThreeFingers) {
                        // En. more than three finger are pressed. Ok. we eat it.
                        return true;
                    }
                }
                break;
            ...
            case MotionEvent.ACTION_POINTER_UP: {
                if (mAllowThreeFingersSlidingForScreenShot) {
                    ...
                    if (mAreThreeFingerssSliding || mHasEatThreeFingers) {
                        if (mDbgSlidingForScreenShot) {
                            Log.d(TAG, "ACTION_POINTER_UP: Eat it because we have intercepted three fingers.");
                        }
            ...
            case MotionEvent.ACTION_MOVE: {
                if (mAllowThreeFingersSlidingForScreenShot) {
                    if (mAreThreeFingerssSliding || mHasEatThreeFingers) {
                        if (mAreThreeFingerssSliding && !mHasStartedScreenShot && ev.getPointerCount() == 3) {                            
                            // Note: Current three fingers maybe are different from previous that we have recorded.
                            //       But at most conditions it can work normally.
                            if (ev.getY(0) - mFirstPointStartY > mSlidingDistanceThreshold
                                    && ev.getY(1) - mSecondPointStartY > mSlidingDistanceThreshold
                                    && ev.getY(2) - mThirdPointStartY > mSlidingDistanceThreshold) {
                                mHasStartedScreenShot = true;
                                
                                if (mDbgSlidingForScreenShot) {
                                    Log.d(TAG, "ACTION_MOVE: Start screen shot...");
                                }
                                Context context = getContext();
                                final Intent intent = new Intent(ACTION);
                                context.sendBroadcast(intent);
            ...
            case MotionEvent.ACTION_UP:{
				...
				fanhuix2 = ev.getX();  
                fanhuiy2 = ev.getY();
				Runtime runtime = Runtime.getRuntime();Log.d("zhangrui","getWidth()="+getWidth()+"fanhuix1="+fanhuix1);
				/*if(fanhuiy1 - fanhuiy2 < 15) {  
				...
				} else*/ if(fanhuix1 - fanhuix2 < 10 && fanhuix1 > getWidth()-10 && fanhuix1 < getWidth()-5) {  
				...
				} else if(fanhuix2 - fanhuix1 < 10 && fanhuix1 > 5 && fanhuix1 < 10) {  
            ...
            case MotionEvent.ACTION_CANCEL: {
                if (mAllowThreeFingersSlidingForScreenShot) {
                    if (mAreThreeFingerssSliding || mHasEatThreeFingers) {
                        if (mDbgSlidingForScreenShot) {
                            Log.d(TAG, "ACTION_UP|ACTION_CANCEL: Eat it because we have intercepted three fingers.");
                        }
        ...
        protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
            ...
            /// M: BMW. [ALPS02530232]
            final int orignWidthSize = MeasureSpec.getSize(widthMeasureSpec);
            ...
            if (!fixedWidth && widthMode == AT_MOST) {
                ...
                if (tv.type != TypedValue.TYPE_NULL) {
                    ...
                    if (width < min) {
                        ...
                        /// M: BMW. [ALPS02530232] Limit the width to the original
                        /// width measure spec. @{
                        if (MultiWindowProxy.isSupported()) {
                            if (MultiWindowProxy.getInstance()
                                    .isFloatingByWinToken(getWindowToken())) {
                                widthMeasureSpec = MeasureSpec.makeMeasureSpec(
                                        Math.min(min, orignWidthSize), EXACTLY);
                            }
                        }
                        /// @}
                        measure = true;
        ...
        private WindowInsets updateColorViews(WindowInsets insets, boolean animate) {
            ...
            if (!mIsFloating /*&& ActivityManager.isHighEndGfx()*/) {  //modified by tangan
        ...
        public void onWindowFocusChanged(boolean hasWindowFocus) {
            ...
            /// M: BMW @{
            if (MultiWindowProxy.isSupported()) {
                if (MultiWindowProxy.getInstance().isFloatingByAppToken(getAppToken())) {
                    setFloatDecorFocus(hasWindowFocus);
                    // [ALPS02454767] Update stick view when focus changed.
                    updateStickView(MultiWindowProxy.getInstance().isSticky(getAppToken()));
                }
            }
            /// @}
        }
    ...
    protected ViewGroup generateLayout(DecorView decor) {
        ...
        /// M: BMW.
        boolean isMultiWindow = MultiWindowProxy.getInstance().isFloatingByAppToken(getAppToken());
        if (mIsFloating) {
        ...
        if(PrizeOption.PRIZE_STATUSBAR_INVERSE_COLOR) {
            TypedArray ta = context.obtainStyledAttributes(new int[] {android.R.attr.statusBarInverse});
            int statusBarInverse = ta.getInt(0, -1);
            ta.recycle();            
            if(statusBarInverse >= 0) {
                params.statusBarInverse = statusBarInverse;
            }

            ta = context.obtainStyledAttributes(new int[] {android.R.attr.autoInverse});
            int autoInverse = ta.getInt(0, -1);
            ta.recycle();
            if(autoInverse >= 0) {
                params.statusBarAutoInverse = autoInverse;
            }

            Log.d(TAG, "statusBarInverse=" + params.statusBarInverse + ", statusBarAutoInverse=" + params.statusBarAutoInverse);
        } else {
            params.statusBarInverse = -1;
        } //end...
        ...
        if (getContainer() == null) {
            if (mBackgroundDrawable == null) {
                if (mBackgroundResource == 0) {
                    ...
                    // solve bug-20361. avoid a white space in video chatting ui for wechat.
                    // prize-linkunhui-20160819
                    if (PrizeOption.PRIZE_SUPPORT_NAV_BAR_FOR_MBACK_DEVICE && 
                            context.getClass().getName().equals("com.tencent.mm.plugin.voip.ui.VideoActivity")) {
        ...
        /// M: BMW. Add float decor for top-level window. @{
        if (MultiWindowProxy.isSupported()
                && !mIsFloating && getContainer() == null
                && isMultiWindow) {
...
/*
    @Override
    public void setWindowType(IBinder appToken, int windowType) {
        mAppToken = appToken;
        if (windowType == MultiWindowProxy.FLOATING_WINDOW_FULL) {
            mMultiWindow = true;
        }
    }
    ...
    public void setFloatDecorVisibility(int visibility) {
        ...
        /// M: [ALPS01940665] Update sticy when FloatDecor's Visibility changed.
        updateStickView(MultiWindowProxy.getInstance().isSticky(mAppToken));
    }
*/

    /// M: BMW. Return the view with the multi window decoration (ex. control bar)
    private ViewGroup prepareFloatDecor(Context context, View mView) {
        ...
        ViewGroup contentView = (ViewGroup) rootView.findViewById(
                com.mediatek.internal.R.id.content);
        contentView.addView(mView, new ViewGroup.LayoutParams(
                ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));

        mActionView = (ViewGroup) rootView.findViewById(
                com.mediatek.internal.R.id.action_bar_container);
        mActionView.setVisibility(View.VISIBLE);

        PackageManager pm = context.getPackageManager();
        ImageView logoView = (ImageView) rootView.findViewById(
        com.mediatek.internal.R.id.activityLogo);
        logoView.setBackgroundDrawable(pm.getApplicationIcon(context.getApplicationInfo()));

        ImageView closeView = (ImageView) rootView.findViewById(
        com.mediatek.internal.R.id.activityClose);
        closeView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
               Log.v(TAG, "[BMW] onCloseFloatingWindow");
               MultiWindowProxy.getInstance().closeWindow(getAppToken());
            }
        });

        ImageView maximumView = (ImageView) rootView.findViewById(
        com.mediatek.internal.R.id.activityMaximum);
        maximumView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
               MultiWindowProxy.getInstance().restoreWindow(getAppToken(), true);
            }
        });

        mStickView = (ImageView) rootView.findViewById(
        com.mediatek.internal.R.id.activityStick);
        ...
        mStickView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                MultiWindowProxy.getInstance().stickWindow(getAppToken(), mSticked);
            }
        });
    ...
    private void updateStickView(boolean isSticky) {
        ...
        mSticked = isSticky;
        if (isSticky) {
            mStickView.setImageResource(
                    com.mediatek.internal.R.drawable.float_window_stick_clicked);
        } else {
            mStickView.setImageResource(
                    com.mediatek.internal.R.drawable.float_window_stick);
        }
    }
    ...
    public void setFloatDecorFocus(boolean hasWindowFocus) {
        ...
        if (hasWindowFocus) {
            mActionView.setBackgroundDrawable(getContext().getResources().getDrawable(
                    com.mediatek.internal.R.drawable.float_window_actionbar_resume));
        } else {
            mActionView.setBackgroundDrawable(getContext().getResources().getDrawable(
                    com.mediatek.internal.R.drawable.float_window_actionbar_pause));
        }
    }
